...
function Robot(x, y) {
	...
	this.need_to_warn_others_of_heavy_mineral_found = false;
	this.mineralWarned = [];
	...
	this.execute = function(){
		...
		if(false){
		//	(NEW) IF need to warn others of heavy mineral found 
		//		AND NOT at the base 
		//		THEN drop 3 crumbs 
		//		AND travel up gradient 
		} else if(this.need_to_warn_others_of_heavy_mineral_found && collisionMothership === -1){
			
			this.followMothershipSignal();
			listTracks.push(new Track(this.drawable.x, this.drawable.y, 3));
				
		//	(NEW) IF need to warn others of heavy mineral found 
		//		AND at the base 
		//		THEN stop warn others 
		} else if(this.need_to_warn_others_of_heavy_mineral_found && collisionMothership != -1){
		...
		//	(2) IF carrying samples AND at the base THEN drop samples
			...
			if(this.mineralLoaded.mustBePushed()){
				if(allRobotsInBase(this.mineralLoaded.packedBy)){
		...
		//		AND NOT at the base 
		//		THEN pick sample up or warn others
			...
			if(collisionMineral != -1){
				...
				if(collisionMineral != -1 && mineral_in_colision !== undefined && mineral_in_colision.packedBy.length === 0){
					...
					if( mineral_in_colision.mustBePushed() ){
						//we need to warn? or we had warned about it?
						//it has other robot pushing?
						if(mineral_in_colision.packedBy.length > 0){
						...
						//If there is no other robot pushing, we should warn about this mineral?
						} else if( !this.mineralWarnedListHas(mineral_in_colision) && collisionTracks.length === 0){
							this.need_to_warn_others_of_heavy_mineral_found = true;
							this.mineralWarned.push(mineral_in_colision);
						// In other case, start help signal
						} else{
							mineral_in_colision.packedBy.push(this);
							this.helpSignal(mineral_in_colision);
						}
	...
	this.mineralWarnedListHas = function(mineral){
		for (i = 0; i < this.mineralWarned.length; i++) {
			if(mineral.equals(this.mineralWarned[i])){
				return true;
			}
		}
		return false;
...
function allRobotsInBase( robots ){
	if(robots.length !== 0){
		if(collisionRobot_Mothership(robots[0]) < 0){
			return false;
		}
		if(collisionRobot_Mothership(robots[1]) < 0){
			return false;
		}
	}
	return true;
